...
# Windows asyncio fix: set the correct event loop policy
if sys.platform == "win32":
    asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())

# Allow nested asyncio loops (needed for Streamlit + Playwright)
nest_asyncio.apply()
...
if uploaded_file is not None:
    df_original, df_exploded = process_uploaded_file(uploaded_file)
    if df_original is None or df_exploded is None:
        st.stop()
else:
    st.info("👈 Please upload an MMS Sales Tracker Excel file using the sidebar to begin.")
    st.stop()

# Compute overall metrics from original data (no splitting)
total_sales = df_original['Amount'].sum()
...
rep_stats_sales = rep_stats.sort_values('total_sales', ascending=False)

# Sort for invoices chart (by invoice count descending)
rep_stats_inv = rep_stats.sort_values('invoice_count', ascending=False)
...
# Chart 1: Sales by Sales Rep (top) – sorted by total sales descending
st.subheader("💰 Sales by Sales Rep (Largest to Smallest)")
fig_sales = px.bar(
    rep_stats_sales,
    x='Sales Rep Number',
    y='total_sales',
    color_discrete_sequence=['#e6550d'],
    text=rep_stats_sales['total_sales'].round(2),
...
# ----- TABLE SECTION -----
st.subheader("📋 Sales Rep Summary (Sorted by Total Sales)")
# Format the dataframe for display
display_df = rep_stats_sales.copy()
...
# Download button for the summary (using per-rep stats)
csv = rep_stats_sales.to_csv(index=False).encode('utf-8')
st.download_button(
...
def generate_export_html():
    # Convert rep_stats_sales to HTML table
    table_html = rep_stats_sales.copy()
        ...
        <div class="container">
            ...
            <div class="metrics-container">
                <div class="metric-card blue">
                    <div class="metric-label">💰 Total Sales (actual)</div>
                    <div class="metric-value">${total_sales:,.2f}</div>
                </div>
                <div class="metric-card green">
                    <div class="metric-label">📄 Total Invoices (actual)</div>
            ...
            <div class="chart-container">
                <h3>💰 Sales by Sales Rep (Largest to Smallest)</h3>
                {sales_html}
            </div>
            <div class="chart-container">
                <h3>📊 Invoices by Sales Rep (Largest to Smallest)</h3>
                {invoices_html}
            </div>
            <div class="table-container">
                <h3>📋 Sales Rep Summary (Sorted by Total Sales)</h3>
                {table_html}
            </div>
        </div>
    </body>
...
# Add HTML export button to sidebar
with st.sidebar:
    st.markdown("---")
    st.download_button(
...
# ----- PDF EXPORT FUNCTIONALITY (using HTML string) -----
...
def export_as_pdf():
    """Generate PDF from the current dashboard HTML."""
